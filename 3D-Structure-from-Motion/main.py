...
def find_features(image_0, image_1):
    ...
    matches = bf.knnMatch(desc_0, desc_1, k=2) # find the two best matches for each descriptor using k nearest neighbors
    feature = []
    for m, n in matches:
...
def run(img_dir:str,apply_bundle_adjustment:boolean=False):
    ...
    feature_0 = feature_0[em_mask.ravel() == 1] # select only inlier points using Ransac mask
    ...
    # cv2.recoverPose() takes the essential matrix and the matching inlier points and extracts the relative camera pose between two images.
